...
export function useSoundFX(): UseSoundFXReturn {
  ...
  // Bell — sine fundamental + inharmonic partials for casino-ding sparkle
  ...
  // Bright chime with slight detune shimmer
  ...
  // Sparkle cascade — random bright pings across a window
  ...
  const play = useCallback((name: SoundName) => {
    ...
    switch (name) {
      ...
      case 'chipClink':
        ...
        break;
      case 'hover':
        // subtle whoosh — soft, non-intrusive
  ...
  const startAmbient = useCallback(() => {
    ...
    if (!ctx) return;
    ensureBus(ctx);
    // Rich Am7 low pad — root, fifth, seventh, tenth — with slow LFO shimmer
    ...
    // Sparse melodic bell — pentatonic runs every 5-9 seconds so it feels alive
  ...
  // Auto-start ambient on first user gesture — browsers require a gesture to
  // unlock audio, so we defer to pointerdown/keydown and start once.
  useEffect(() => {
