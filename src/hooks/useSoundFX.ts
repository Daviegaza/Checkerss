...
const STORAGE_KEY = 'checkers_sound_v2';
...
export function useSoundFX(): UseSoundFXReturn {
  ...
  const ensureBus = useCallback((ctx: AudioContext) => {
    ...
    for (let ch = 0; ch < 2; ch++) {
      ...
      for (let i = 0; i < len; i++) {
        const t = i / len;
        d[i] = (Math.random() * 2 - 1) * Math.pow(1 - t, 2.8) * (0.7 + 0.3 * Math.sin(t * 12));
      }
    }
  ...
  const startAmbient = useCallback(() => {
    ...
    if (!ctx) return;
    ensureBus(ctx);
    const freqs = [110, 165, 196, 262, 330];
    const nodes = freqs.map((f, i) => {
      ...
      gain.gain.linearRampToValueAtTime(0.032 - i * 0.004, ctx.currentTime + 3.5);
      const lfo = ctx.createOscillator();
      lfo.type = 'sine';
      lfo.frequency.setValueAtTime(0.08 + Math.random() * 0.14, ctx.currentTime);
      const lfoGain = ctx.createGain();
      lfoGain.gain.setValueAtTime(0.014, ctx.currentTime);
      lfo.connect(lfoGain).connect(gain.gain);
      osc.connect(gain);
      connectBus(ctx, gain, 1.2);
      osc.start();
      lfo.start();
  ...
  // Ambient does NOT auto-start — opt-in only via toggleAmbient. Prior behavior
  // auto-kicked a droning pad on first gesture; users reported it as noise.
  useEffect(() => {
